...
public class PlayerMover : MonoBehaviour
{
    ...
    private void MovePlayerToComputedPositions()
    {
        if (bShouldMoveToDesired && Vector2.Distance(transform.position, vDesiredPosition) > fPlayerPositionBias)
        {
            ...
            spriteRenderer.flipX = vDesiredPosition.x < transform.position.x;
        }
        else
        {
    ...
    private void MovePlayerWithKeyboard()
    {
        ...
        if (!bShouldMoveToDesired)
        {
            animator.SetFloat("moveX", Mathf.Abs(horizontalAspect));
            spriteRenderer.flipX = horizontalAspect < 0;
        }
